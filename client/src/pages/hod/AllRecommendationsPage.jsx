...
export function AllRecommendationsPage({ items, filterPriority = "all", onOrderChange, onSubmit, isPeriodOpen, currentPeriod }) {
  ...
  const filteredItems = filterPriority === "high" ? activeItems.filter((item) => item.priorityRank === 1) : activeItems;
  const title = filterPriority === "high" ? "Top Ranked Recommendation" : "Department Recommendation Order";
  const allAssigned = activeItems.length > 0 && activeItems.every((item) => Number.isFinite(item.priorityRank));
